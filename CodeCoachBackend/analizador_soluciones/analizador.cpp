 ...
 * obtener feedback, an�lisis de complejidad y tipo de algoritmo, y lo devuelve al cliente.
 *
...
/**
 * Funci�n de parsing manual MEJORADA para extraer el JSON anidado de la respuesta de Gemini.
 ...
 * @return El string JSON interno (que contiene nuestro feedback), o un string vac�o si falla.
 */
std::string extract_json_text_from_gemini_response(const std::string& gemini_body) {
    ...
    while (end < gemini_body.length()) {
        if (gemini_body[end] == '\\') {
            ...
            if (backslashes % 2 == 0) {
                // Esta comilla NO est� escapada (precedida por 0, 2, 4... backslashes)
                // Es el final de nuestro string.
                break;
            }
            // La comilla S� est� escapada (precedida por 1, 3, 5... backslashes)
    ...
    if (end == gemini_body.length()) {
        return ""; // Se lleg� al final sin encontrar la comilla de cierre
    }
...
// Construye la petici�n al LLM (Gemini), la env�a y devuelve el JSON de feedback que genera el LLM.
// - user_code: c�digo fuente enviado por el estudiante
...
std::string get_llm_feedback(const std::string& user_code, const std::string& eval_results) {
    ...
    if (api_key.empty()) {
        ...
        return "{\"feedback\": \"Error del servidor: La clave API de Google no est� configurada.\", \"analysis_algorithm\": \"Error\", \"analysis_complexity_time\": \"Error\", \"analysis_complexity_space\": \"Error\"}";
    }
    ...
    std::string system_prompt =
        "Eres un 'Code Coach' experto en C++. Tu trabajo es analizar el c�digo de un estudiante y los resultados de sus pruebas. "
        "NUNCA debes dar la soluci�n completa; solo pistas, explicaciones de errores y sugerencias de mejora. Mant�n el reto. "
        "Responde �NICAMENTE con un objeto JSON (sin ```json ni ning�n otro texto introductorio). "
        "El JSON debe tener la siguiente estructura exacta: "
        "{"
        "  \"feedback\": \"Tu pista o explicaci�n breve y amigable aqu�.\","
        "  \"analysis_algorithm\": \"El nombre del algoritmo o patr�n que identificaste (ej. 'B�squeda Lineal', 'Programaci�n Din�mica', 'N/A' si no es claro).\", "
        "  \"analysis_complexity_time\": \"Tu estimaci�n de la complejidad de tiempo (ej. 'O(n)', 'O(n^2)').\", "
        "  \"analysis_complexity_space\": \"Tu estimaci�n de la complejidad de espacio (ej. 'O(1)', 'O(n)').\""
        "}";

    // user_prompt: contiene el c�digo del alumno y los resultados de las pruebas.
    std::string user_prompt =
        "Mi c�digo:\n```cpp\n" + user_code + "\n```\n\n"
        "Resultados de las pruebas:\n" + eval_results + "\n\n"
        "Por favor, dame tu an�lisis en el formato JSON solicitado.";
    ...
    // Aqu� ponemos el prompt dentro de "contents" -> "parts" -> "text".
    std::string payload =
    ...
    // Enviamos POST con el payload; la librer�a devuelve un objeto opcional con la respuesta.
    if (auto res = cli.Post(url, payload, "application/json")) {
        if (res->status == 200) {
            ...
            if (feedback_json_string.empty()) {
                ...
                std::cerr << "Error parseando respuesta de Gemini (no se encontr� 'text' o estaba mal formado):\n" << res->body << std::endl;
                return "{\"feedback\": \"Error: Respuesta inv�lida del Coach AI (JSON anidado no encontrado).\", \"analysis_algorithm\": \"Error\", \"analysis_complexity_time\": \"Error\", \"analysis_complexity_space\": \"Error\"}";
            }

            // --- INICIO DE LA L�GICA DE LIMPIEZA ---

            // Gemini devuelve la cadena con escapes (\n, \", \\). Aqu� los convertimos a sus caracteres reales.
            size_t pos = 0;
            ...
            // Limpia espacios en blanco o saltos de l�nea al inicio y al final
        ...
        // No se recibi� respuesta: error de red. 'res' no existir�, pero la librer�a permite
        // consultar el error a trav�s de res.error().
        auto err = res.error();
        std::cerr << "Error de red: " << httplib::to_string(err) << std::endl;
        return "{\"feedback\": \"Error: Falla de conexi�n con el Coach AI.\", \"analysis_algorithm\": \"Error\", \"analysis_complexity_time\": \"Error\", \"analysis_complexity_space\": \"Error\"}";
    }
}
...
int main() {
    ...
    svr.Post("/analyze", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            // Como la funci�n de LLM ya devuelve un JSON (como string),
